  ...
  try {
    ...
    const updatedVendor = await prisma.$transaction(async (tx) => {
      ...
      if (openingBalance !== undefined) {
        ...
        /* 2A️⃣ Create account if missing */
        if (!accountId) {
          const account = await tx.account.create({
            data: {
        ...
        /* 2B️⃣ Prevent change if transactions exist */
        ...
        /* 2C️⃣ Update or create opening balance entry */
    ...
    if (err.message === "OPENING_LOCKED")
      return res.status(409).json({
        success: false,
        error: "Opening balance cannot be changed once transactions exist",
      });
